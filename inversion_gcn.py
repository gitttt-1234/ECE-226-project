...
def inverse(dataset,modelname, lr_f, lr_stru, n, params, net, feature_paras, targets, criterion, criterion_stru, self_loop, degree_as_label, epochs, batch_size, 
    ...
    savefolder = './save/fakegraphs'+'_'+str(dataset)+'_bn'+str(bn_reg_scale)+'_oh'+str(onehot_cof)+'/'
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='PyTorch Fake Graph Generation')
    ...
    parser.add_argument('--dataset', type=str, help='name of dataset (default: MUTAG)')
    ...
    parser.add_argument('--batch_size', type=int, help='batch size')#188 = 169+19

    parser.add_argument('--total_num', type=int, help='number of fake graphs, defualt 131 on MUTAG, 188*0.7=131.6')
    ...
    for bat_num in range(total_batchnum):
        ...
        if bat_num!=total_batchnum-1 or args.total_num%args.batch_size==0:
            ...
            for n in number_list:
                ...
                f = np.random.normal(size = (n, dataset.dim_nfeats))#np.ones((n, 7)).astype(np.float32)
            ...
            cost, best_strus = inverse(dataset=args.dataset,modelname = args.modelt, lr_f=args.di_lr_f, lr_stru=args.di_lr_stru, n=sample_n, 
            ...
            generate_graphs(args.dataset,best_strus, features, targets, savept, args.dataseed, args.trial, args.modelt, bat_num, args.total_num)
